...
class NotificationService extends BaseService with PeriodicServiceMixin {
  ...
  Future<void> performPeriodicTask() async {
    if (_notifications.isNotEmpty) {
      ...
      logger.info('Processing notification', metadata: {'notification': notification});
      _processedCount++;
    }
  }
...
class EnhancedServiceLocator extends ServiceLocator {
  ...
  void _injectDependencies() {
    ...
    if (isServiceInitialized<UserService>() && isServiceInitialized<DatabaseService>()) {
    ...
    if (isServiceInitialized<UserService>() && isServiceInitialized<CacheService>()) {
      final userService = get<UserService>();
...
Future<void> main() async {
  ...
  try {
    ...
    final user1Cached = await userService.getUserProfile('123'); // Should come from cache
    ...
    print('   Sent 2 notifications (queue size: ${notificationService.queueSize})');
    ...
    for (final info in allInfo) {
      print('   ${info.name}: ${info.state.name} (${info.dependencies.length} deps)');
    }
    print('');
    ...
    final warningLogs = logEntries.where((e) => e.level == ServiceLogLevel.warning);
